...
/**
  * @FileName : AccountConsultingExpensesExcel.java
  * @Project : ilgaja
  * @Date : 2020. 12. 16. 
  * @작성자 : Jangjaeho
  * @변경이력 :
  * @프로그램 설명 : 상담사 통계 엑셀
  */
public class AccountConsultingExpensesExcel extends AbstractXlsxStreamingView {
...
//  		sheet1.setColumnView(5, 20);
		 ...
		 */
  		...
  		adminName 	= (String) model.get("adminName");
...
//  		numberFormat2.setBackground(Colour.GRAY_25);                    	// 배경색 설정
  	...
  	private void createLabelRow(Sheet sheet,  AccountingDTO accountingDTO) throws Exception {
  		String title = accountingDTO.getStartDate().substring(0, 7) +"월 " + adminName;
...
//	    sheet.setColumnView(1, 25);
	    ...
	    row2.createCell(2).setCellValue("상담사명");
  		row2.getCell(2).setCellStyle(titleFormat1);
  		sheet.setColumnWidth(2, (short)(50*256));
//	    sheet.addCell(new jxl.write.Label(2, rowNum,   "상담사명", titleFormat1));
//	    sheet.setColumnView(2, 50);
	    
	    row2.createCell(3).setCellValue("지점매출");
  		row2.getCell(3).setCellStyle(titleFormat1);
  		sheet.setColumnWidth(3, (short)(40*256));
//	    sheet.addCell(new jxl.write.Label(3, rowNum,   "지점매출", titleFormat1));
//	    sheet.setColumnView(3, 40);
	    row2.createCell(4).setCellValue("상담사수수료");
  		row2.getCell(4).setCellStyle(titleFormat1);
  		sheet.setColumnWidth(4, (short)(40*256));
//	    sheet.addCell(new jxl.write.Label(4, rowNum,   "상담사수수료", titleFormat1));
...
//  			sheet.addCell(new jxl.write.Label(++colNum, rowNum,   nal, cellFormat2));
  			row.createCell(colNum).setCellValue(item.getAdmin_name());
			row.getCell(colNum++).setCellStyle(cellFormat2);
//  			sheet.addCell(new jxl.write.Label(++colNum, rowNum,   item.getAdmin_name(), cellFormat2));
...
//  			sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getIlbo_fee()), numberFormat));
  			row.createCell(colNum).setCellValue(Integer.parseInt(item.getCounselor_fee()));
			row.getCell(colNum++).setCellStyle(numberFormat);
//  			sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getCounselor_fee()), numberFormat));
...
//  			sheet.setRowView(rowNum, 400);
  			...
  			totalCounselorFee += Integer.parseInt(item.getCounselor_fee());
  			rowNum++;	  
  		}
...
//  		sheet.addCell(new jxl.write.Number(3, rowNum,   totalIlboFee, numberFormat2));
  		row.createCell(4).setCellValue(totalCounselorFee);
  		row.getCell(4).setCellStyle(numberFormat2);
//  		sheet.addCell(new jxl.write.Number(4, rowNum,   totalCounselorFee, numberFormat2));
  		row.setHeight((short)400);
//  		sheet.setRowView(rowNum, 400);
  	}
