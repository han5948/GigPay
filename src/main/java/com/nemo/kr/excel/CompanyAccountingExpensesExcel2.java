...
public class CompanyAccountingExpensesExcel2 extends AbstractXlsxStreamingView {
...
//		numberFormat2.setBackground(Colour.GRAY_25);                    	// 배경색 설정
	...
	private void createLabelRow(Sheet sheet,  AccountingDTO accountingDTO) throws Exception {
		...
		//sheet.mergeCells(0, rowNum, 5, rowNum); // 시작열, 시작행, 종료열, 종료행
...
//	    sheet.addCell(new jxl.write.Label(2, rowNum,   "정산 지점", titleFormat1));
//	    sheet.setColumnView(2, 50);
	    
	    sheet.setColumnWidth(3, (short)(40*256));
		row2.createCell(3).setCellValue("구인정산 수수료");
  		row2.getCell(3).setCellStyle(titleFormat1);
//	    sheet.addCell(new jxl.write.Label(3, rowNum,   "구인정산 수수료", titleFormat1));
//	    sheet.setColumnView(3, 40);
	    
	    sheet.setColumnWidth(4, (short)(40*256));
		row2.createCell(4).setCellValue("구직정산 수수료");
  		row2.getCell(4).setCellStyle(titleFormat1);
//	    sheet.addCell(new jxl.write.Label(4, rowNum,   "구직정산 수수료", titleFormat1));
//	    sheet.setColumnView(4, 40);
	    
	    sheet.setColumnWidth(5, (short)(40*256));
		row2.createCell(5).setCellValue("정산금 합계");
  		row2.getCell(5).setCellStyle(titleFormat1);
//	    sheet.addCell(new jxl.write.Label(5, rowNum,   "정산금 합계", titleFormat1));
//	    sheet.setColumnView(5, 40);
	...
	private void createDataRow(Sheet sheet, AccountingDTO accountingDTO, List<AccountingDTO> resultList) throws Exception {
		...
		int totalWorkSettlementFee = 0;
		int totalWorkerSettlementFee =0;
		int totalFee = 0;
		float totalAcc = 0;
...
//		float worker_fees = (float) (Integer.parseInt(Const.worker_fees) * 0.01);
		...
		for (int i = 0; i < resultList.size(); i++) {
			...
			float work_fee_rate = (float) (Integer.parseInt(item.getWork_fee_rate()) * 0.01);
			float worker_fee_rate = (float) (Integer.parseInt(item.getWorker_fee_rate()) * 0.01);
			...
			int work_income = Integer.parseInt(item.getWork_income());
			int work_payment = Integer.parseInt(item.getWork_payment());
			int worker_income = Integer.parseInt(item.getWorker_income());
			int worker_payment = Integer.parseInt(item.getWorker_payment());
			float workSettlementFee = (work_income + work_payment) * work_fee_rate;
			float workerSettlementFee = (worker_income + worker_payment) * worker_fee_rate;
			
			totalAcc = workSettlementFee + workerSettlementFee;
			...
			row.getCell(colNum++).setCellStyle(numberFormat);
			//sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getIlbo_fee()) * work_fees, numberFormat));
			row.createCell(colNum).setCellValue(workerSettlementFee);
			row.getCell(colNum++).setCellStyle(numberFormat);
			//sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getShare_fee()) * worker_fees, numberFormat));
		...
		totalFee = totalWorkSettlementFee + totalWorkerSettlementFee;
