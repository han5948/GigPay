...
/**
  * @FileName : AccountingAdExpensesExcel.java
  * @Date : 2020. 12. 18. 
  * @작성자 : Na GilJin
  * @프로그램 설명 : 광고료 통계 엑셀
  */
public class AccountingAdListExcel extends AbstractXlsxStreamingView {
...
//	    sheet.setColumnView(1, 25);
	    ...
	    row2.createCell(2).setCellValue("매출");
	    row2.getCell(2).setCellStyle(titleFormat1);
	    sheet.setColumnWidth(2, (short)(50*256));
//	    sheet.addCell(new jxl.write.Label(2, rowNum,   "매출", titleFormat1));
//	    sheet.setColumnView(2, 50);
	    
	    row2.createCell(3).setCellValue("수수료");
	    row2.getCell(3).setCellStyle(titleFormat1);
	    sheet.setColumnWidth(3, (short)(40*256));
//	    sheet.addCell(new jxl.write.Label(3, rowNum,   "수수료", titleFormat1));
//	    sheet.setColumnView(3, 40);
	    row2.createCell(4).setCellValue("네이버 광고료");
	    row2.getCell(4).setCellStyle(titleFormat1);
	    sheet.setColumnWidth(4, (short)(40*256));
//	    sheet.addCell(new jxl.write.Label(4, rowNum,   "네이버 광고료", titleFormat1));
...
//			sheet.addCell(new jxl.write.Number(colNum, rowNum,   (i+1), numberFormat));
			row.createCell(colNum).setCellValue(item.getIlbo_date());
			row.getCell(colNum++).setCellStyle(cellFormat2);
//			sheet.addCell(new jxl.write.Label(++colNum, rowNum,   item.getIlbo_date(), cellFormat2));
			row.createCell(colNum).setCellValue(Integer.parseInt(item.getTotal_sales()));
			row.getCell(colNum++).setCellStyle(numberFormat);
//			sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getTotal_sales()), numberFormat));
...
//			sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getIlbo_fee()), numberFormat));
			row.createCell(colNum).setCellValue(Integer.parseInt(item.getAd_price()));
			row.getCell(colNum++).setCellStyle(numberFormat);
//			sheet.addCell(new jxl.write.Number(++colNum, rowNum,   Integer.parseInt(item.getAd_price()), numberFormat));
			...
			totalTotalSale += Integer.parseInt(item.getTotal_sales());
			totalIlboFee += Integer.parseInt(item.getIlbo_fee());
			totalAdPrice += Integer.parseInt(item.getAd_price());
			rowNum++;	  
		}
	  
		sheet.addMergedRegion(new CellRangeAddress(rowNum, rowNum, 0, 1));
		//sheet.mergeCells(0, rowNum, 1, rowNum); // 시작열, 시작행, 종료열, 종료행
		Row row = sheet.createRow(rowNum);
		...
		row.createCell(2).setCellValue(totalTotalSale);
		row.getCell(2).setCellStyle(numberFormat2);
//		sheet.addCell(new jxl.write.Number(2, rowNum,   totalTotalSale, numberFormat2));
...
//		sheet.addCell(new jxl.write.Number(3, rowNum,   totalIlboFee, numberFormat2));
		row.createCell(4).setCellValue(totalAdPrice);
		row.getCell(4).setCellStyle(numberFormat2);
//		sheet.addCell(new jxl.write.Number(4, rowNum,   totalAdPrice, numberFormat2));
		row.setHeight((short)400);
//		sheet.setRowView(rowNum, 400);
	}
