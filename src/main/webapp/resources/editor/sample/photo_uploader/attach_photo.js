...
//		bSupportDragAndDropAPI = false;
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/resources/editor/file_uploader_html5.jsp'; 	//upload URL
//    	sUploadURL= '/admin/testimg'; 	//upload URL
    	//파일을 하나씩 보내고, 결과를 받음.
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						//생성된 파일 file_info 테이블에 저장
						callAjaxFileInfo(tempFile.name, res._response.responseText.substring(res._response.responseText.indexOf("#name;")+6), tempFile.size);
    ...
    function callAjaxFileInfo(file_org_name, file_name, file_size){
    	var oAjax = jindo.$Ajax("/admin/boardUploadImage", {
			type: 'xhr',
			method : "post",
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
			},
		...
		oAjax.header("contentType","text/html");
		oAjax.header("file-org-name",file_org_name);
		oAjax.header("file-name",file_name);
		oAjax.header("file-size",file_size);
		oAjax.request();
    }
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			if( aSubTemp[0] == "sFileURL" ){
	    				aSubTemp[1] = aSubTemp[1].replace(/;/gi, "=");
	    				aSubTemp[1] = aSubTemp[1].replace(/#/gi, "&");
	    			}
