...
function toastW(text){//주의
	$.toast({title: text, position: 'middle', backgroundColor: '#00CC00', duration:2000 });
}

function toastOk(text){//성공
	$.toast({title: text, position: 'middle', backgroundColor: '#5aa5da', duration:1000 });
}


function toastFail(text){//실패
	$.toast({title: text, position: 'middle', backgroundColor: '#d60606', duration:2000 });
}
...
$.extend({
	...
	*/
	toast: function(obj) {
		...
		if(flag) {
			var content = "<div id='toast-div' style='position: fixed;display: none; z-index:9999;font-size: 12px; " + position + ";left: 0;width:100%; height: " + height + "; text-align: center'>";
		} else {
			var content = "<div id='toast-div' style='position: fixed; display: none;z-index:9999; top: 0; left: 0;width:100%; height:100%; text-align: center'>";
		}
