...
"button"===a?"button":"textbox";k=document.createElement("input");k.type=a;f(k,c);k.value=d;"text"==a&&(b(k).select());"button"!==a&&(e?c.size||b(k).css({width:"100%","box-sizing":"border-box"}):c.size||(c.size=20));b(k).attr("role",g);break;case "image":case "file":k=document.createElement("input");k.type=a;f(k,c);break;case "custom":k=document.createElement("span");try{if(b.isFunction(c.custom_element)){var B=c.custom_element.call(l,d,c);if(B instanceof jQuery||t.isHTMLElement(B)||"string"===typeof B)B=b(B).addClass("customelement").attr({id:c.id,
