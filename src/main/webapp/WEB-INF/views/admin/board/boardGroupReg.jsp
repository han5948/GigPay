...
<script type="text/javascript">
	...
	function insertProcess() {
		if ( chkValidEmpty('board_name') && chkValidEmpty('board_comment') ) {
			$('#frmReg').ajaxForm({
				url : "/admin/board/boardGroupRegProcess",
				dataType : "json",
		...
		<table class="bd-form" summary="게시판등록.">
			...
			<tbody>
				...
				<tr>
					...
					<td class="linelY">
						<input type="radio" name="email_yn" value="1" checked="checked" title="사용유무" /> 사용
						<input type="radio" name="email_yn" value="0" title="사용유무"  />미사용 
					</td>
				</tr>
				<tr>
					<th scope="col">phone</th>
					<td class="linelY">
						<input type="radio" name="phone_yn" value="1" checked="checked" title="사용유무" /> 사용
						<input type="radio" name="phone_yn" value="0" title="사용유무"  />미사용 
					</td>
				</tr>
				<tr>
					<th scope="col">글쓰기 지점</th>
					<td class="linelY">
						<input type="radio" name="write_company_seq" value="0" checked="checked" title="글쓰기 지점" /> 전체
						<input type="radio" name="write_company_seq" value="13" title="글쓰기 지점"  />본사
					</td>
				</tr>
				<tr>
					<th scope="col">글쓰기 권한</th>
					<td class="linelY">
						<input type="radio" name="write_admin_auth" value="-1" checked="checked" title="글쓰기 권한" /> 전체
						<input type="radio" name="write_admin_auth" value="0" title="글쓰기 권한"  />최고관리자
						<input type="radio" name="write_admin_auth" value="1" title="글쓰기 권한"  />관리자이상
					</td>
				</tr>
			</tbody>
		</table>
