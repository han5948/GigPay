...
$(function() {
  ...
  $("#list").jqGrid({
            ...
            // 컬럼명
            colNames: ['순번', '부모지점순번' ,'지점순번', '지점', '구인처순번','구인처','사업자번호',  '담당자타입', '본사매니저이름', '본사매니저폰번호', '비밀번호', '메모', '계정관리','OSType','push유무','pushToken', '가입경로', '상태', '최근접속일시','방문횟수','접속방법','등록일시', '등록자','소유자'],
...
//                                                                                                          return { label: item.code + ":" + item.name, value: item.code, id: item.id }
                 ...
                 {name: 'employer_seq'   , index: 'employer_seq', width: 60, hidden: authLevel == 0 ? false : true },
                 {name: 'employer_name'  , index: 'employer_name'  , align:   "left", sortable:  true 
								...
								dataInit: function (e, cm) {
                                     ...
                                     $(e).autocomplete({
											...
											select: function (event, ui) {
												...
												$("#list").jqGrid('setCell', lastsel,'employer_num', ui.item.employer_num, '', true);  //다른 셀 바꾸기
					...
					, editrules: {edithidden: true, required: false, custom:  true, custom_func: validEmployerName}
					, searchoptions: {sopt: ['cn','eq','nc']}
					, formatter: formatSelectName
              },
              
              {name: 'employer_num'   , index: 'employer_num'   , align:   "left", sortable:  true, searchoptions: {sopt: ['cn','eq','nc']} , editable:  false
            	  , formatter :formatRegNoStar
            	  , editrules: {custom:  true, custom_func: validBizID}
              ...
              {name: 'manager_type'   , index: 'manager_type',  hidden: authLevel == 0 ? false : true,   align: "center", width:  60, search: false, editable:  true, sortable:  true, edittype: "select", editoptions: {value: manager_type_opts}, formatter: "select",
                     cellattr: function(rowId, tv, rowObject, cm, rdata) {
                 ...
                 {name: 'manager_name'   , index: 'manager_name'   , align:   "left", sortable:  true, searchoptions: {sopt: ['cn','eq','nc']}
                  	, editable:  true
                  	, cellattr: function(rowId, tv, rowObject, cm, rdata){
                 ...
                 {name: 'manager_phone'  , index: 'manager_phone'  , align:   "left", editable:  true, sortable:  true
                	 , formatter: formatPhoneStar, searchoptions: {sopt: ['cn','eq','nc']}
                 	 , editrules: {custom:  true, custom_func: validManagerPhone},
                 },
                 {name: 'manager_pass'  , index: 'manager_pass'  , align:   "left", editable:  true, search: false
	              		, cellattr: function(rowId, tv, rowObject, cm, rdata) {
	                  		return 'style="background-color: #f7f4e4;"';
						}
             	},
                 {name: 'manager_memo'   , index: 'manager_memo'   , align:   "left", editable:  true, sortable:  true, edittype: "textarea", searchoptions: {sopt: ['cn','eq','nc']}},

                 {name: 'manager_use_status', index: 'manager_use_status', align: "center", width: 100, editable: true, sortable: true, edittype: "select", editoptions: {value: use_status_opts}, formatter: "select" , stype:"select", searchoptions: {sopt: ['eq'], value:search_use_status_opts},
 				},
                 {name: 'os_type'           , index: 'os_type'   , align: "center", width:  60,   sortable:  true, edittype: "select", editoptions: {value: os_type_opts}, formatter: "select", stype:"select", searchoptions: {sopt: ['eq'], value:search_os_type_opts},
                     cellattr: function(rowId, tv, rowObject, cm, rdata) {
                                 // rowObject 변수로 그리드 데이터에 접근
                 ...
                 {name: 'manager_push_yn'           , index: 'manager_push_yn'   , align: "center", width:  60, editable:  true, sortable:  true, edittype: "select", editoptions: {value: use_opts}, formatter: "select", stype:"select", searchoptions: {sopt: ['eq'], value:search_use_opts},
                       cellattr: function(rowId, tv, rowObject, cm, rdata) {
                                   // rowObject 변수로 그리드 데이터에 접근
                 ...
                 {name: 'visit_date'     , index: 'visit_date'     , align: "center", width: 150, search: false, editable: false, sortable:  true, formatoptions: {newformat: "y/m/d H:i"}},
           ...
           /*             
                       if(cellName == "employer_pass"){
                    	   var employerNum = $("#list").jqGrid('getRowData', rowid).employer_num;
                    	   
                    	   if(employerNum  == "" || employerNum == "?" ){
                    		    $("#list").jqGrid('setCell', rowid, cellName, "", 'not-editable-cell'); // 특정 cell 수정 못하게
                    		    alert("사업자 번호를 먼저 등록 하셔야 됩니다.");
                   	   			return true;
                    	   }else{
                    		 	$("#"+rowid).removeClass("not-editable-cell"); //해제
...
//                        $("#totalRecords").html(numberWithCommas( $("#list").getGridParam("records") ));
	...
	$("#btnESMS").on("click", function() {
		...
		var params = smsPopAdd('10');
		
	 	if(params.length == 0) {
	 		$.toast({title: '본사매니저의 번호가 없거나 선택된 본사매니저가 없습니다.', position: 'middle', backgroundColor: '#ef7220', duration:1000 });
		}else {
			var text = '';
...
function fn_setEditable(rowId, managerCompanySeq){
	...
	if(!checkMyManager(rowId)){
		...
		$("#list").jqGrid('setCell', rowId,  'employer_pass', "", 'not-editable-cell');
...
function smsPopAdd(tr_etc1) {
	...
	for(var i = 0; i < idArray.length; i++) {
		if($("input:checkbox[id=jqg_list_" + idArray[i] + "]").is(":checked")) {
			...
			if((tr_etc1 == '10' && rowData.manager_phone.replace(/\s/g, "").length != 0 && rowData.ilbo_worker_phone != '?')) { 
				paramData = {
...
function validManagerPhone(phoneNum, nm, valref) {
  ...
  $.ajax({
		...
		success : function(data) {
			...
			if( data.result == "0000" ){
	        	if( data.resultDTO ){
	        		if(data.resultDTO.manager_type == "E"){
	        			valiResult = [false, "이미 등록된 폰 번호입니다."];
	        		}else if(data.resultDTO.manager_type == "O"){
	        			valiResult = [false, "현장매니저로 가입된 번호입니다."];	
	        		}
	        	}
	        }
		},
				...
				<td>
					<p class="floatL">
						<input type="text" id="start_reg_date" name="start_reg_date"
							class="inp-field wid100 datepicker" /> <span class="dateTxt">~</span>
						<input type="text" id="end_reg_date" name="end_reg_date"
							class="inp-field wid100 datepicker" />
					</p>
					<div class="inputUI_simple">
						<span class="contGp btnCalendar"> <input type="radio"
							id="day_type_1" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', 'all'   ); $('#btnSrh').trigger('click');"
							checked="checked" /><label for="day_type_1"
							class="label-radio on">전체</label> <input type="radio"
							id="day_type_2" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', 'today' ); $('#btnSrh').trigger('click');" /><label
							for="day_type_2" class="label-radio">오늘</label> <input
							type="radio" id="day_type_3" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', 'week'  ); $('#btnSrh').trigger('click');" /><label
							for="day_type_3" class="label-radio">1주일</label> <input
							type="radio" id="day_type_4" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', 'month' ); $('#btnSrh').trigger('click');" /><label
							for="day_type_4" class="label-radio">1개월</label> <input
							type="radio" id="day_type_5" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', '3month'); $('#btnSrh').trigger('click');" /><label
							for="day_type_5" class="label-radio">3개월</label> <input
							type="radio" id="day_type_6" name="day_type" class="inputJo"
							onclick="setDayType('start_reg_date', 'end_reg_date', '6month'); $('#btnSrh').trigger('click');" /><label
							for="day_type_6" class="label-radio">6개월</label>
						</span> 
					</div>
				</td>

				<th scope="row" class="linelY pdlM">상태</th>
				<td><input type="radio" id="srh_use_yn_1" name="srh_use_yn"
					class="inputJo" value="" /><label for="srh_use_yn_1"
					class="label-radio">전체</label> <input type="radio"
					id="srh_use_yn_2" name="srh_use_yn" class="inputJo" value="1"
					checked="checked" /><label for="srh_use_yn_2" class="label-radio">사용</label>
					<input type="radio" id="srh_use_yn_3" name="srh_use_yn"
					class="inputJo" value="0" /><label for="srh_use_yn_3"
					class="label-radio">미사용</label></td>
					
				<th scope="row" class="linelY pdlM">직접검색</th>
				<td>
					<div class="select-inner">
						<select id="srh_type" name="srh_type"
							class="styled02 floatL wid138" onChange="$('#srh_text').focus();">
<!-- 							<option value="" selected="selected">전체</option> -->
							<c:if test="${sessionScope.ADMIN_SESSION.auth_level eq 0}">
								<option value="company_name">지점</option>
							</c:if>
							<option value="employer_name">구인처</option>
							<option value="employer_num">사업자번호</option>

						</select>
					</div> <input type="text" id="srh_text" name="srh_text"
					class="inp-field wid300 mglS"
					onKeyDown="if ( event.keyCode == 13 ) $('#btnSrh').click();" />
...
<div class="btn-module mgtS mgbS">
	<div class="leftGroup">
		...
		<c:if test="${sessionScope.ADMIN_SESSION.company_seq eq '13' and sessionScope.ADMIN_SESSION.auth_level eq '1' }">
			<a href="#none" id="btnESMS" style="width: 97px;" class="btnStyle01">본사매니저 SMS</a>
			</c:if>
